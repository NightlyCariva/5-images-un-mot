...
int main(int argc, char *argv[])
{
    ...
    SDL_bool run = SDL_TRUE;    //Game loop
    SDL_bool isCloseHold = SDL_FALSE; //boutton close (Hold = Souris passer dessus ou non )
    SDL_bool isSoundClick = SDL_FALSE; //boutton son
    SDL_bool isOkHold = SDL_FALSE; //boutton ok
    SDL_bool dontPut = SDL_FALSE;   //ne pas réccuperer la saisi (pour les boutton qui sert a des commandes)
    ...
    int ScoreP1 = 1;    // je ne peux pas faire de 0 en score dès le départ la conversion int -> char ne se fait pas pour 0, solution ? j'initialise à 1 et je n'affiche le score que quand on aura besoin 
    int ScoreP2 = 1;
    Arbre* root2 = NULL;   //arbre pour partie a deux joueurs (je sais je peux utilisé optimisé en utilisant un seul arbre)


    //ImagesV2
    char adresseImageV2[1000];
    int imageNumber = 1; // les images sont nommée comme suit 1,2,3,4,5 (on commencera par l'image numéro 1)

    //DEBUG MODE:
    int ZenMod = 0; //0 : ne fait rien , 1 : Active tous les modes debug, -1 : desactive tous les modes debug
    SDL_bool MouseDebug = SDL_TRUE; //Permet de voir les coordonées liée au mouvement de la souris sur la fenetre du jeu
    SDL_bool ResponsiveDebug = SDL_TRUE;  //permet de voir les nouveaux size de l'ecran a chaque redimensionnement
    SDL_bool GodMod = SDL_TRUE; //permet de Tout savoir: sur la partie en cours, l'adresse de l'image,le mot dans les deux langues choisi
    SDL_bool ClickedDebug = SDL_TRUE; //permet de verifié sur la console si on a bien clické sur le boutton en question
    SDL_bool PressDebug = SDL_TRUE; //permet de savoir si une touche/commande rapide est appuyé
    SDL_bool InputDebug = SDL_TRUE; //permet de voir les saisie au clavier
    SDL_bool StructureDebug = SDL_TRUE; //permet de voir le chargement de la structure
    SDL_bool StructureLoadChrono = SDL_TRUE; //Permet de voir en combien de temps la structure s'est fait charger
    SDL_bool TentativeDebug = SDL_TRUE; //permet de voir le nombre de tentative + score fait par chaque joueur
    SDL_bool SwitchImageDebug = SDL_TRUE;  //permet de savoir si on a bien switcher d'image pour le meme mot + l'adresse de la nouvelle
    ...
    {
        MouseDebug = SDL_TRUE;
        ResponsiveDebug = SDL_TRUE;
        GodMod = SDL_TRUE;
        ClickedDebug = SDL_TRUE;
        PressDebug = SDL_TRUE;
        InputDebug = SDL_TRUE;
        StructureDebug = SDL_TRUE;
        StructureLoadChrono = SDL_TRUE;
        TentativeDebug = SDL_TRUE;
        SwitchImageDebug = SDL_TRUE;
    }
    else if (ZenMod == -1)
    {
        MouseDebug = SDL_FALSE;
        ResponsiveDebug = SDL_FALSE;
        GodMod = SDL_FALSE;
        ClickedDebug = SDL_FALSE;
        PressDebug = SDL_FALSE;
        InputDebug = SDL_FALSE;
        StructureDebug = SDL_FALSE;
        StructureLoadChrono = SDL_FALSE;
        TentativeDebug = SDL_FALSE;
        SwitchImageDebug = SDL_FALSE;
    }
    ...
    if (ResponsiveDebug) printf("old size w : %d h : %d\n",WIDTH_FENETRE,HEIGHT_FENETRE);
    ...
    //Button Previous
    SDL_Rect dimPrevious = {0,0,0,0};
    SDL_Rect dimPreviousAfter = {0,0,60,60};
    //Button Next
    SDL_Rect dimNext = {0,0,0,0};
    SDL_Rect dimNextAfter = {0,0,60,60};
    ...
    //Next
    SDL_Texture* nextTexture = loadImage(pathNext,render);
    if(GestionError(nextTexture,"Erreur au chargement du boutton Next")==statut) goto Quit;
    SDL_QueryTexture(nextTexture,NULL,NULL,&dimNext.w,&dimNext.h);
    dimNextAfter.x= dimImageBoardAfter.x + dimImageBoardAfter.w -160;
    dimNextAfter.y= dimImageBoardAfter.y + dimImageBoardAfter.h/2 - dimNextAfter.h/2;

    //Previous
    SDL_Texture* previousTexture = loadImage(pathPrevious,render);
    if(GestionError(previousTexture,"Erreur au chargement du boutton Previous")==statut) goto Quit;
    SDL_QueryTexture(previousTexture,NULL,NULL,&dimPrevious.w,&dimPrevious.h);
    dimPreviousAfter.x= dimImageBoardAfter.x +100;
    dimPreviousAfter.y= dimImageBoardAfter.y + dimImageBoardAfter.h/2 - dimPreviousAfter.h/2;
    ...
    while(run)
    {
            ...
            switch (events.type)
            {
                case SDL_WINDOWEVENT: //Boutton de fenetre
                    ...
                    else if (events.window.event == SDL_WINDOWEVENT_RESIZED)
                    {
                        ...
                        SDL_GetWindowSize(fenetre,&WIDTH_FENETRE,&HEIGHT_FENETRE);
                        if (ResponsiveDebug) printf("new size w : %d h : %d\n",WIDTH_FENETRE,HEIGHT_FENETRE);
                        //Menu
                        menuPosAfter.x = WIDTH_FENETRE /2 - menuPosAfter.w /2;
                        menuPosAfter.y = HEIGHT_FENETRE /2 - menuPosAfter.h /2 +90;
                        //Option
                ...
                case SDL_MOUSEBUTTONUP: //Au relachement du boutton de la souris
                    //Next Effect
                    if(isMouseOnRect(events.button.x,events.button.y,&dimNextAfter))
                    {
                        if(wichOptionClicked == 1 || wichOptionClicked == 2)
                        {
                            if (imageNumber < 5) imageNumber++;
                            else imageNumber = 1;   // effet de rotation
                            if (ClickedDebug) printf("Boutton Next clické, Numéro d'image : %d\n",imageNumber);
                            strcpy(adresseImageV2,PATHIMAGES);
                            strcat(adresseImageV2,AdressAbsolutImage);
                            strcat(adresseImageV2,"/");
                            strcat(adresseImageV2,itoa(imageNumber,10));
                            strcat(adresseImageV2,".png");
                            if(SwitchImageDebug) printf("DEBUG : CHANGEMENT IMAGE FULL ADRESSE DETECTED : %s\n",adresseImageV2);
                            imageInside = loadImage(adresseImageV2,render);
                    ...
                    //Previous Effect
                    if(isMouseOnRect(events.button.x,events.button.y,&dimPreviousAfter))
                    {
                        if(wichOptionClicked == 1 || wichOptionClicked == 2)
                        {
                            if (imageNumber > 1) imageNumber--;
                            else imageNumber = 5;
                            if(ClickedDebug) printf("Boutton Previous clické, Numéro d'image : %d\n",imageNumber);
                        }
                        strcpy(adresseImageV2,PATHIMAGES);
                        strcat(adresseImageV2,AdressAbsolutImage);
                        strcat(adresseImageV2,"/");
                    ...
                    if (isMouseOnRect(events.button.x,events.button.y,&dimTextAreaAfter))
                    {
                        if(wichOptionClicked == 1 || wichOptionClicked == 2)
                        {
                            ...
                            if(ClickedDebug) printf("textArea clicked\n");
                        }
                    }
                    if(isMouseOnRect(events.button.x,events.button.y,&dimOkAfter)) //Retour en arrière
                    {
                        if (wichOptionClicked == 1 ||wichOptionClicked == 2) //si on est en jeu on initialise le chargement de structure
                        {
                            ...
                            if(ClickedDebug) printf("Back clicked\n");
                        }
                        wichOptionClicked = 0;
                    }
                    //Boutton Apropos
                    if(isMouseOnRect(events.button.x, events.button.y,&dimOp3)) //option 3 clické
                    {
                        ...
                        {
                            wichOptionClicked = 3;
                            if(ClickedDebug) printf("Option Apropos clicked\n");
                        }
                    }
                    else if (isMouseOnRect(events.button.x, events.button.y,&dimOp2)) //option 2 clické
                    {
                        ...
                        {
                            wichOptionClicked = 2;
                            if(ClickedDebug) printf("Option Multi clicked\n");
                        }
                    }
                    else if (isMouseOnRect(events.button.x,events.button.y,&dimOp1)) //option 1 clické
                    {
                        ...
                        {
                            wichOptionClicked = 1;
                            if(ClickedDebug) printf("Option Solo clicked\n");
                        }
                    }
                    //flag selection
                    if (wichOptionClicked == 0)
                ...
                case SDL_KEYDOWN: //Appuis sur une touche
                    ...
                    {
                        if (PressDebug) printf("boutton UP appuyer\n");
                        volumeUp(&volume);
                    ...
                    {
                        if (PressDebug) printf("boutton DOWN appuyer\n");
                        volumeDown(&volume);
                        Mix_VolumeMusic(volume);
                        printf("volume : %d\n",volume);
                        dontPut = SDL_TRUE;
                    ...
                    {
                        if (PressDebug) printf("boutton echap appuyer\n");
                    ...
                    if ( events.key.keysym.sym == SDLK_RETURN && strlen(InputText) > 0) //Appuyer sur Entrer et y'a de l'input
                    {
                        ...
                        {
                            if (PressDebug) printf("Entrer Appuyer !\n");
                    ...
                    if (wichOptionClicked == 1) // on n'accept les input que lorsqu'on est en jeu SOLO
                    {
                        if( !( SDL_GetModState() & KMOD_CTRL && ( events.text.text[ 0 ] == 'c' || events.text.text[ 0 ] == 'C' || events.text.text[ 0 ] == 'v' || events.text.text[ 0 ] == 'V' ) ) )
                        {
                            if (!(dontPut))
                            {
                                ...
                                if (InputDebug) printf("inputAvant : %s + Lettre Saisie : %s\n",InputText,events.text.text);
        ...
        else if (wichOptionClicked == 2) //Multi clické
        {
            ...
            if (! StructLoaded){
                root2 = NULL;
                fileTab = NULL ;            //Tableau de tous les mots de notre fichier
                FrTab = NULL;
                struct timespec start, stop;    //variable de temps de chargement de structure
                clock_gettime(CLOCK_REALTIME, &start); //Let the show beging ! 
                //Chargement de la structure ...
                count_mots = 0; // reinitialisation (on sait jamais)
                fileTab = FileToTable("wordsV22.txt");   //fichier doit se terminer avec un séparateur en plus (il marque un pointeur vers une chaine NULL) 
                ...
                for (int i = 0; fileTab[i] != NULL; i++)
                {
                    ...
                    if(StructureDebug) printf("[%d] : MotFr : %s, Deuxième Langue (%d) {0:FR,1:ES,2:EN}: %s, AdresseImage : %s \n",i,v->MotFr,(isFlagClicked-1),v->Mot2Langue,v->AdresseImage);
                    ...
                    FreeTab2D(splitTab,4); //le tableau de split contient 4 partie "," on s'endebarasse plus besoin
                }
                ...
                if (StructureDebug) printf("Suggestion : %s\n",suggestionMots);
                ...
                posSugg.x = WIDTH_FENETRE/2 - posSugg.w/2 ;
                posSugg.y = dimScoreTwoPlayer1After.h + 20;
                clock_gettime(CLOCK_REALTIME, &stop); //the show is ended :D 
                if(StructureLoadChrono)
                {
                    printf("\n========================================================================================\n");
                    printf("Temps de chargement : %.9lfs\n",(double) ((stop.tv_nsec * 1e-9 + stop.tv_sec) - (start.tv_nsec * 1e-9 + start.tv_sec)));
                    printf("\n========================================================================================\n");
                }
                StructLoaded = SDL_TRUE; //On arrete de load la structure !
                SearchPhase = SDL_FALSE; //qui sait ?(s'assurer c bien)
                PlayerTurn = 1; // toujours le premier joueur qui va faire chercher le 2 eme 
                //Free du garbage (on a plus besoin de ca, seul l'arbre compte)
                FreeTab2D(FrTab,count_mots);
                FreeTab2D(fileTab,count_mots);
            }
            ...
            if (ReturnPressed) //l'un des joueurs a appuyer sur entré 
            {
                ...
                {   
                    ...
                    if (RechercheAVL(root2,RechercheMot,MotLangueChoisie,AdressAbsolutImage) == 1)
                    {
                        ...
                        strcpy(InputText,"Saisir un mot");
                        if (GodMod) printf("le mot attendu est : %s, l'adresse image sortie est : %s\n",MotLangueChoisie,AdressAbsolutImage);
                        strcpy(adresseImageV2,PATHIMAGES);
                        strcat(adresseImageV2,AdressAbsolutImage);
                        strcat(adresseImageV2,"/");
                        strcat(adresseImageV2,itoa(imageNumber,10));
                        strcat(adresseImageV2,".png");
                        if (SwitchImageDebug) printf("DEBUG : IMAGE FULL ADRESSE : %s\n",adresseImageV2);
                        imageInside = loadImage(adresseImageV2,render);
                        if(GestionError(imageInside,"Erreur au chargement du conteneur de l'image en jeu")==statut) goto Quit;
                        SDL_QueryTexture(imageInside,NULL,NULL,&dimImageInside.w,&dimImageInside.h);
                        dimImageInsideAfter.x = dimImageBoardAfter.x + (dimImageBoardAfter.w - dimImageInsideAfter.w)/2;
                    ...
                    {
                        if (StructureDebug) printf("mot rechercher introuvable dans la bdd {%s}\n",InputText);
                ...
                else
                {
                    ...
                    {
                        tentative++;
                        if (TentativeDebug) printf("tentative encours num : %d",tentative);
            ...
            {
                //[Debug]
                if (TentativeDebug) printf("tentative gagné num : %d",tentative);
                if (PlayerTurn == 2)
                {
                    ScoreP2 += Score(tentative,100);
                    if (TentativeDebug) printf("le score du joueur {2} est : %d\n",ScoreP1);//[Debug]
            ...
            if (PlayerTurn == 1)
            {
                ...
                fontPlayer2 = insertText(MENU_POLICE, 24,fontColorWhite,"[Joueur 2]:", render);
            }
            ...
            SDL_RenderCopy(render,previousTexture,&dimPrevious,&dimPreviousAfter);
            SDL_RenderCopy(render,nextTexture,&dimNext,&dimNextAfter);
        ...
        {   
            if (! StructLoaded )
            {
                ...
                for (int i = 0; fileTab[i] != NULL; i++)
                {
                    ...
                    Enregistrement* v = newValeur(splitTab[0],splitTab[isFlagClicked-1],splitTab[3]); // création de l'enregistrement
                    if(StructureDebug) printf("[%d] : MotFr : %s, Deuxième Langue (%d) : %s, AdresseImage : %s \n",i,v->MotFr,(isFlagClicked-1),v->Mot2Langue,v->AdresseImage);
                ...
                /*=======================================================================================================================================================================*/
                    ...
                    FrTab = (char**)realloc(FrTab,sizeof(char*)*(cpt_NbrMot+1));
                    FrTab[cpt_NbrMot] = NULL; //tableau se terminant avec un champs null    
                    //Plus besoin de splitTab
                    FreeTab2D(splitTab,4);
                } 
                ...
                {
                    if(StructureDebug) printf("Une erreur au tirage d'un mot aleatoire : le mot tiré (%s) n'existe pas en AVL \n",FrTab[indexFr]);
                    goto Quit;
                }
                if(GodMod) printf("le mot attendu est : %s, l'adresse image sortie est : %s\n",MotLangueChoisie,AdressAbsolutImage);
                strcpy(adresseImageV2,PATHIMAGES);
            ...
            //render le text si on veut
                ...
                if(ReturnPressed)
                {
                    ...
                    {
                        tentative++;
                        if(TentativeDebug) printf("tentative encours num : %d\n",tentative);
                        strcpy(InputText,"Mauvaise reponse !");
                        fontMenu = insertText(MENU_POLICE, 24, fontColorError,InputText, render);
                        position.x = dimTextAreaAfter.x + (dimTextAreaAfter.w - position.w)/2 ;
                        position.y = dimTextAreaAfter.y + (dimTextAreaAfter.h/2 - position.h)/2;
                ...
                if(isWinning)
                {
                    ...
                    if(TentativeDebug) printf("le score du joueur est : %d\n",ScoreSolo);
    ...
    ============================================================*/
Quit:
    if (root != NULL) FreeAVL(root); //pas besoin de la condition elle est deja verifié dans la fonction Free mais bon mieux s'assurer
    if (root2 != NULL) FreeAVL(root2);
    //SDL QUIT
    if (nextTexture != NULL) SDL_DestroyTexture(nextTexture);
    if (previousTexture != NULL) SDL_DestroyTexture(previousTexture);
